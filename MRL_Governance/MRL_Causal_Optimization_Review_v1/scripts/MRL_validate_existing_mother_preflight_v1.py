...
def main(path):
    ...
    if missing_chain:
        raise SystemExit(f"missing lookup chain entries: {sorted(missing_chain)}")
    refs = data.get("notion_refs", {})
    missing_refs = REQUIRED_REFS - set(refs)
    if missing_refs:
        raise SystemExit(f"missing notion refs: {sorted(missing_refs)}")
    if any(not str(refs[k]).startswith("https://app.notion.com/p/") for k in REQUIRED_REFS):
        raise SystemExit("invalid notion ref")
    op = data.get("operation")
    if op not in ALLOWED:
        raise SystemExit("invalid operation")
    if op in {"MAP_EXISTING", "SUPPLEMENT_EXISTING"} and data.get("parallel_mother_created") is not False:
        raise SystemExit("existing-node operations may not create a parallel mother")
    if data.get("create_new_requires_no_existing_node") is not True:
        raise SystemExit("CREATE_NEW must require no existing node")
    if data.get("backfill_required_after_create") is not True:
        raise SystemExit("backfill after create must be mandatory")
    if data.get("origin_signature") != "MrLiouWord":
        raise SystemExit("origin signature mismatch")
    print("MRL_EXISTING_MOTHER_PREFLIGHT_PASS")
    return 0
