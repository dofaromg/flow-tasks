...
> 來源：使用者提供之 particle_core 本地執行說明（對應外部 repo `dofaromg/FlowAgent.Runtime` 的 `particle_core/`）。
...
> `dofaromg/FlowAgent.Runtime`（不在本工作 repo 權限範圍）。本檔僅為規格/操作說明留位。
...
## 子系統（外部 FlowAgent.Runtime/particle_core）
...
2. 授權/匯入 `dofaromg/FlowAgent.Runtime`（目前不在本 repo 權限範圍）。
3. 明確指示「依本規格在 mrl_ai_system 內實作 particle_core」——
   ⚠ 注意：這會與 FlowAgent.Runtime 既有實作產生**第二套並存實作**的分歧風險（同 BaseWorld 27 問題），需你確認是否接受。
