const MODULE_NAME = 'mrl-firecore-live';
const FIREBASE_COUNTERPART = 'Firestore Listeners';
const ORIGIN_SIGNATURE = 'MrLiouWord';
const PRIORITY = 'P2';
const BATCH = 'Batch 054';
...
function health(env: Env): Response {
  return J({
    ...
    endpoints: ["/health", "/v1/live/stream", "/v1/live/ws", "/v1/live/topics/:topic"]
  });
}

async function liveContract(path: string) {
  return J({
    ok: true,
    module: MODULE_NAME,
    path,
    accepted: false,
    reason: 'local contract response only; live stream depends on DL580 event bus',
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (path.startsWith('/v1/live')) return liveContract(path);
