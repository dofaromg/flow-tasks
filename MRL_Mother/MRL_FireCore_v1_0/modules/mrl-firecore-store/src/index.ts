const MODULE_NAME = 'mrl-firecore-store';
const FIREBASE_COUNTERPART = 'Firestore';
...
function health(env: Env): Response {
  return J({
    ...
    endpoints: ["/health", "/v1/store/documents", "/v1/store/documents/:collection/:id", "/v1/store/query"]
  });
}

async function storeContract(path: string, body: unknown) {
  return J({
    ok: true,
    module: MODULE_NAME,
    path,
    accepted: false,
    reason: 'local contract response only; authoritative write requires DL580 origin_signature',
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (path.startsWith('/v1/store')) return storeContract(path, body);
