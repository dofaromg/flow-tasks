const MODULE_NAME = 'mrl-firecore-push';
const FIREBASE_COUNTERPART = 'Cloud Messaging / FCM';
...
function health(env: Env): Response {
  return J({
    ...
    endpoints: ["/health", "/v1/push/register", "/v1/push/send", "/v1/push/topics"]
  });
}

async function pushContract(path: string, body: unknown) {
  return J({
    ok: true,
    module: MODULE_NAME,
    path,
    accepted: false,
    reason: 'local contract response only; dispatch policy must be signed by DL580',
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (path.startsWith('/v1/push')) return pushContract(path, body);
