# MrLiouAI CodePartner 人格回收包 / Recovery Manifest
...
| `MrLiouAI.SystemPlan.FullStack.v1.txt` | MrLiouAI 系統完整架構說明書 — 列出四人格（Fluin / EchoBody / CodePartner / SeedPersona） |
| `MrLiouAI_語場語言系統建構大綱_2025-07-23.txt` | 粒子語言（.fltnz / .flpkg）建構大綱，含語場封存來回對等原則 |
...
| `seed_modules/FlowSeed.Total.v1.qflpkg` | MrLiouAI 總體種子封包 —「語場起源宇宙壓縮核」：七層系統報告書（L1 系統總覽 ～ L7 語義記憶網）＋ FlowSeed 雙 Manifest — 第四輪回收，SHA256 `a7ea263055fa4c56…` 驗證一致 |
...
| `seed_modules/MrLiouAI_系統白皮書.pdf` | MrLiouAI 系統白皮書（47.5KB PDF）— SHA256 `34708ddcf3fb24a0…` 驗證一致 |

## 回收路徑 / Provenance chain

1. 原始 `.flpkg` 封包（`MrLiouAI.TotalCore.Unity.v1.flpkg` 等）僅存在於
   ChatGPT 對話期下載檔，未曾進入任何 git repo。
2. `dofaromg/flow-tasks` 保存了運行證據：
   `MrLiouAI_Unity_v3_高維模擬檢查報告.txt` 記錄
