...
`DELIVERY_PASS` means the GitHub package is present, non-empty, checksummed and its loopback integration tests pass. It does **not** mean the DL580 model is currently reachable.

`MRL_AI_MOTHER_AUTONOMOUS_RUNTIME_ACCEPTANCE_PASS` requires a real local Ollama or llama.cpp model to be running on DL580 and the PowerShell acceptance flow to pass. Until then, the physical runtime Gate remains `OPEN`.

## Quick start on DL580
